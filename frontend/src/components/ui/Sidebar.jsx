...
export function SidebarMenuItem({ children, className, onClick = () => { }, to = "", isExternal = false }) {
    ...
    return (
        <li onClick={handleClick}>
            <Wrapper
                className={cn("hover:bg-base-content/5 p-2 rounded-field cursor-pointer flex items-center gap-2", className)}
                {...wrapperProps}
            >
                {children}
            </Wrapper>
...
export function CollapsibleSidebarSubMenu({ children, className, items }) {
    ...
    return (
        <li>
            <div className={cn("hover:bg-base-content/5 p-2 rounded-field cursor-pointer flex items-center", className)} onClick={toggleIsOpen}>
